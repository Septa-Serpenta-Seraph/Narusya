...
#
# Pipeline: Qdrant snapshot -> collect -> encrypt secrets ->
#           gzip state.db -> chunk (>95M) -> git commit/force-push
# ============================================================
set -uo pipefail
...
# Core collections get point-in-time snapshots. SNAP name below is FIXED
# per collection so git delta compression works across runs.
...
# ---- 1. Passphrase (create if missing, else reuse) ----
...
# ---- 2. Qdrant snapshots (snapshot one collection at a time) ----
...
rsync -a "$HOME/daemon-work/" "$WORK_DIR/daemon/" 2>/dev/null
