...
DATE_RE = re.compile(
    r"(?:##.*?)"
    r"(?:"
    r"(?P<m1>[A-Z][a-z]{2})\s+(?P<d1>\d{1,2}),\s+(?P<y1>\d{4})\s*~(?P<t1>\d{1,2}:\d{2})"
    r"|"
    r"(?P<y2>\d{4})-(?P<m2>\d{2})-(?P<d2>\d{2})\s+(?P<t2>\d{1,2}:\d{2})"
    r"|"
    r"(?P<m3>[A-Z][a-z]{2})\s+(?P<d3>\d{1,2})\s+(?P<y3>\d{4})\s*~(?P<t3>\d{1,2}:\d{2})"
    r")"
    r".*?UTC"
)
...
def main():
    ...
    lines.append("")
    lines.append("PEN-FELL %s — daemon ran (bot posts present) but the log-append "
